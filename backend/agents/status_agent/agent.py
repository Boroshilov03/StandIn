...
async def _run_brief_pipeline(ctx: Context, msg: FullBriefRequest) -> FullBriefResponse:
    ...
    roles = [r for r in requested_roles if r in ROLE_CONFIGS]
    unknown_roles = [r for r in requested_roles if r not in ROLE_CONFIGS]
    if unknown_roles:
        ctx.logger.warning(f"Ignoring unknown roles: {unknown_roles}")
    if not roles:
        roles = ["Product"]
